...
@section('content')
    ...
    <div x-data="filterController()" class="mb-6 bg-white rounded-xl border border-gray-200 p-5">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <button type="button" @click="selectPeriod('this_week')"
                    :class="period === 'this_week' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                    This Week
                </button>
                <button type="button" @click="selectPeriod('this_month')"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            @else
                <div id="dailyTrendChart" role="img"
                    aria-label="Daily visit trend showing {{ $dailyTrend->sum('count') }} total visits"></div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            @if ($weeklyTrend === null)
                <div class="text-center py-10">
                    ...
                    <p class="text-sm text-gray-500">{{ session('info_weekly', 'Insufficient data for weekly trend') }}
                    </p>
                </div>
            ...
            @else
                <div id="weeklyTrendChart" role="img"
                    aria-label="Weekly visit trend showing {{ $weeklyTrend->sum('count') }} total visits"></div>
            @endif
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            @if ($monthlyTrend === null)
                <div class="text-center py-10">
                    ...
                    <p class="text-sm text-gray-500">{{ session('info_monthly', 'Insufficient data for monthly trend') }}
                    </p>
                </div>
            ...
            @else
                <div id="monthlyTrendChart" role="img"
                    aria-label="Monthly visit trend showing {{ $monthlyTrend->sum('count') }} total visits"></div>
            @endif
        </div>
    </div>
                ...
                tooltip: {
                    x: {
                        formatter: function(val) {
                            ...
                            return 'Week of ' + date.toLocaleDateString('en-MY', {
                                month: 'short',
                                day: 'numeric',
                                year: 'numeric'
                            });
