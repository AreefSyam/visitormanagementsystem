...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <h2 class="text-base font-semibold text-gray-800 mb-5">Edit — {{ $visitor->name }}</h2>

        <form method="POST" action="{{ route('visitors.update', $visitor) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="company" value="{{ old('company', $visitor->company) }}"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">ID Type <span class="text-red-500">*</span></label>
                    <select name="id_type" required
                        ...
                        @foreach(['ic' => 'IC / MyKad', 'passport' => 'Passport', 'driving_license' => 'Driving License', 'other' => 'Other'] as $val => $label)
                            <option value="{{ $val }}" {{ old('id_type', $visitor->id_type) === $val ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="id_number" value="{{ old('id_number', $visitor->id_number) }}" required
                           class="w-full px-3 py-2 text-sm border rounded-lg outline-none transition
                ...
                <div class="sm:col-span-2">
                    ...
                    @if($visitor->photo)
                        <div class="flex items-center gap-4 mb-3">
                            <img src="{{ Storage::url($visitor->photo) }}" alt="Photo"
